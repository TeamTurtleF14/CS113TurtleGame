		...
		case ZeroGameEngine::GameOver:
			...
			if (showCredits){
				if (!Background.loadFromFile("images/Backgrounds/CreditScreen.png"))
					if (BGM.getStatus()==sf::Music::Playing)
						BGM.stop();
					if (!BGM.openFromFile("sounds/music/WinnerWinner.wav"))
						return;
					BGM.setLoop(true);
					if (BGM.getStatus()!=sf::Music::Playing)
						BGM.play();
...
//	Background.setRepeated(true);
	...
	string music[2] = "sounds/music/ANewDevelopment.wav", "sounds/music/OfBitsAndNibbles.wav"		
			
	if (BGM.getStatus()!=sf::Music::Playing){
		BGM.openFromFile(music[(std::rand() % 2)])
		BGM.play();
		BGM.setLoop() = false;
	}
